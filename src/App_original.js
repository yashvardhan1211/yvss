...
import { listenToSalonUpdates } from './services/salonService';
...
function CustomerApp() {
  ...
  useEffect(() => {
    const setupNotifications = async () => {
      try {
        await initializeNotifications('customer_' + Date.now(), 'customer');
  ...
  // Search for nearby salons (with demo fallback)
  const searchNearbyPlaces = async (location) => {
    console.log('🔍 Searching for salons near:', location);
    
    // First try demo data for quick testing
    console.log('🎭 Using demo salon data for local testing');
    return generateDemoSalons(location);
  };
  ...
  // Original Google Places search (commented out for now)
  const searchRealPlaces = async (location) => {
      ...
      <main className="app-main">
        ...
        <BookingModal
          ...
          onBookingComplete={(bookingData) => {
            ...
            // Here you would typically update the salon's queue
